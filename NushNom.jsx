  ...
  return (
    <div style={{ position: "relative", width: 100, height: 100, flexShrink: 0 }}>
      <svg width="100" height="100" viewBox="0 0 100 100">
        <circle cx="50" cy="50" r={r} fill="none" stroke="#3d2466" strokeWidth="8" />
        <circle
          cx="50" cy="50" r={r} fill="none" stroke="#ffd23f" strokeWidth="8"
      ...
      <div style={{
        ...
        <span style={{ fontFamily: "'Press Start 2P', monospace", fontSize: 13, color: "#fffbe8", fontWeight: 700 }}>{points}</span>
      </div>
    </div>
  );
}
...
function BadgeChip({ badge }) {
  ...
  return (
    <div style={{
      display: "flex", alignItems: "center", gap: 6, background: "#3d2466",
      border: "1px solid #4a2f7a", borderRadius: 2, padding: "6px 12px",
      fontSize: 12, color: "#fffbe8", fontFamily: "'VT323', monospace", fontWeight: 500
    }}>
  ...
  return (
    <div
      ...
      style={{
        ...
        border: "1px solid #4a2f7a", borderRadius: 4, padding: 16,
        cursor: onClick ? "pointer" : "default", position: "relative",
        transition: "transform 0.15s, border-color 0.15s"
      }}
      onMouseEnter={e => { e.currentTarget.style.transform = "translateY(-3px)"; e.currentTarget.style.borderColor = "#ff2e63"; }}
      onMouseLeave={e => { e.currentTarget.style.transform = "translateY(0)"; e.currentTarget.style.borderColor = "#4a2f7a"; }}
    >
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <div>
          <div style={{ fontFamily: "'Press Start 2P', cursive", fontSize: 17, color: "#fffbe8", fontWeight: 700 }}>{restaurant.name}</div>
          <div style={{ fontSize: 12, color: "#8b8bc4", display: "flex", alignItems: "center", gap: 4, marginTop: 2 }}>
            <MapPin size={11} /> {restaurant.area}
          </div>
        </div>
        <div style={{
          background: "#ffd23f", color: "#2e1a00", fontFamily: "'Press Start 2P', monospace",
          fontWeight: 700, fontSize: 13, borderRadius: 2, padding: "4px 8px", flexShrink: 0
        }}>
          {avgScore.toFixed(1)}
        </div>
      </div>
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 10 }}>
        {restaurant.cuisines.map(c => (
          <span key={c} style={{ fontSize: 11, color: "#8b8bc4", background: "#0d0221", border: "1px solid #3d2466", borderRadius: 2, padding: "3px 8px" }}>{c}</span>
        ))}
      </div>
      {topDish && (
        <div style={{ marginTop: 10, fontSize: 12, color: "#ff85a8", display: "flex", alignItems: "center", gap: 5 }}>
...
function PasscodeGate({ onUnlock, onCancel }) {
  ...
  return (
    <div style={{
      background: "#1a0b2e", border: "1px solid #4a2f7a", borderRadius: 4,
      padding: 28, maxWidth: 340, margin: "40px auto", textAlign: "center"
    }}>
      <Lock size={28} color="#ffd23f" style={{ marginBottom: 10 }} />
      <div style={{ fontFamily: "'Press Start 2P', cursive", fontSize: 18, color: "#fffbe8", marginBottom: 6 }}>Curator access only</div>
      <div style={{ fontSize: 13, color: "#8b8bc4", marginBottom: 16 }}>Enter the passcode to add a review.</div>
      <input
        type="password"
        value={val}
        ...
        style={{
          width: "100%", padding: "10px 12px", borderRadius: 3, border: `1px solid ${err ? "#ff2e63" : "#4a2f7a"}`,
          background: "#0d0221", color: "#fffbe8", fontSize: 14, marginBottom: 14, boxSizing: "border-box"
        }}
        onKeyDown={e => { if (e.key === "Enter") { val === PASSCODE ? onUnlock() : setErr(true); } }}
      />
      {err && <div style={{ color: "#ff2e63", fontSize: 12, marginBottom: 10 }}>Wrong passcode, try again.</div>}
...
const btnPrimary = {
  flex: 1, background: "#ff2e63", color: "#3d0d1f", border: "none", borderRadius: 3,
  padding: "10px 14px", fontWeight: 700, fontFamily: "'VT323', monospace", fontSize: 14, cursor: "pointer"
};
const btnGhost = {
  flex: 1, background: "transparent", color: "#8b8bc4", border: "1px solid #4a2f7a", borderRadius: 3,
  padding: "10px 14px", fontWeight: 500, fontFamily: "'VT323', monospace", fontSize: 14, cursor: "pointer"
};
const inputStyle = {
  width: "100%", padding: "10px 12px", borderRadius: 3, border: "1px solid #4a2f7a",
  background: "#0d0221", color: "#fffbe8", fontSize: 14, boxSizing: "border-box", fontFamily: "'VT323', monospace"
};
const labelStyle = { fontSize: 12, color: "#8b8bc4", marginBottom: 6, display: "block", fontWeight: 500 };

function SushiBeltLoader() {
  const items = ["🍣", "🍤", "🍱", "🍙", "🥢", "🍣", "🍤", "🍱", "🍙", "🥢"];
  return (
    <div style={{
      width: "100%", overflow: "hidden", background: "#0d0221", border: "2px solid #4a2f7a",
      borderRadius: 4, padding: "16px 0 10px", marginBottom: 20, position: "relative"
    }}>
      <div style={{
      ...
      <div style={{
        ...
        background: "repeating-linear-gradient(90deg, #ffd23f 0 10px, transparent 10px 20px)"
      }} />
    </div>
  );
}
...
function DishRow({ dish, onChange, onRemove }) {
  ...
  return (
    <div style={{ background: "#0d0221", border: "1px solid #3d2466", borderRadius: 3, padding: 14, marginBottom: 10 }}>
      <div style={{ display: "flex", gap: 10, marginBottom: 10 }}>
        ...
        />
        <button onClick={onRemove} style={{ background: "none", border: "none", color: "#ff2e63", cursor: "pointer" }}><X size={18} /></button>
      </div>
      <div style={{ display: "flex", gap: 12, alignItems: "center", marginBottom: 10 }}>
        <StarRating value={dish.rating} onChange={n => onChange({ ...dish, rating: n })} size={16} />
        <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, color: "#8b8bc4", cursor: "pointer" }}>
          <Camera size={14} /> {dish.photo ? "Change photo" : "Add photo"}
          <input type="file" accept="image/*" onChange={handlePhoto} style={{ display: "none" }} />
        </label>
        {dish.photo && <img src={dish.photo} alt="" style={{ width: 32, height: 32, borderRadius: 2, objectFit: "cover" }} />}
      </div>
      <textarea
  ...
  if (done) {
    ...
    return (
      <div style={{ textAlign: "center", padding: "40px 20px" }}>
        <div style={{ fontFamily: "'Press Start 2P', cursive", fontSize: 18, color: "#fffbe8", marginBottom: 6 }}>Review logged!</div>
        <div style={{ fontSize: 13, color: "#8b8bc4", marginBottom: 20 }}>{restaurant.name} · recommendation score {done.recommendationScore}/5</div>
        <SushiBeltLoader />
        <div style={{ fontFamily: "'Press Start 2P', monospace", fontSize: 13, color: "#ffd23f" }}>
          🍕 Slice {currentSlices === 0 ? SLICES_PER_PIZZA : currentSlices}/{SLICES_PER_PIZZA} added{currentSlices === 0 ? ` · pizza #${wholePizzas} complete!` : ""}
        </div>
        <div style={{ fontSize: 11, color: "#5c5470", marginTop: 16 }}>Heading back to your dashboard...</div>
      </div>
    );
  }
    ...
    return (
      <div>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
          <div style={{ fontFamily: "'Press Start 2P', cursive", fontSize: 20, color: "#fffbe8" }}>Find a restaurant</div>
          <button onClick={onClose} style={{ background: "none", border: "none", color: "#8b8bc4", cursor: "pointer" }}><X size={20} /></button>
        </div>
        ...
        {placesError && (
          <div style={{ marginTop: 10, fontSize: 12, color: "#ff2e63", background: "#2a0d10", border: "1px solid #4a2f7a", borderRadius: 2, padding: "8px 12px" }}>
        ...
        <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 8, maxHeight: 360, overflowY: "auto" }}>
          {filtered.length > 0 && (
            <div style={{ fontSize: 11, color: "#5c5470", textTransform: "uppercase", letterSpacing: 0.5, marginTop: 2 }}>Already on NushNom</div>
          )}
          {filtered.map(r => (
            <div key={r.id} onClick={() => pickRestaurant(r)} style={{
              background: "#0d0221", border: "1px solid #3d2466", borderRadius: 3, padding: "10px 14px",
              cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center"
            }}>
              <div>
                <div style={{ color: "#fffbe8", fontSize: 14, fontWeight: 500 }}>{r.name}</div>
          ...
          {query.trim().length >= 3 && !placesError && (
            <div style={{ fontSize: 11, color: "#5c5470", textTransform: "uppercase", letterSpacing: 0.5, marginTop: filtered.length ? 10 : 2, display: "flex", alignItems: "center", gap: 6 }}>
          ...
          {placesResults
            ...
            .map((p) => (
              <div key={p.placeId} onClick={() => pickPlaceResult(p)} style={{
                ...
                <div>
                  <div style={{ color: "#fffbe8", fontSize: 14, fontWeight: 500 }}>{p.name}</div>
          ...
          {query.trim().length >= 3 && !placesSearching && placesResults.length === 0 && filtered.length === 0 && (
            <div style={{ background: "#0d0221", border: "1px dashed #4a2f7a", borderRadius: 3, padding: 14 }}>
              <div style={{ color: "#8b8bc4", fontSize: 13, marginBottom: 8 }}>Nothing found on NushNom or Google Places. Add it manually.</div>
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
        <button onClick={() => setMode("pick")} style={{ background: "none", border: "none", color: "#8b8bc4", cursor: "pointer" }}><ArrowLeft size={18} /></button>
        <div>
          <div style={{ fontFamily: "'Press Start 2P', cursive", fontSize: 19, color: "#fffbe8" }}>{restaurant.name}</div>
          <div style={{ fontSize: 12, color: "#8b8bc4" }}>{restaurant.area}</div>
        </div>
      </div>
      ...
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 16 }}>
        {CUISINES.map(c => (
          <span key={c} onClick={() => toggleCuisine(c)} style={{
            fontSize: 12, padding: "6px 12px", borderRadius: 2, cursor: "pointer",
            border: `1px solid ${cuisines.includes(c) ? "#ff2e63" : "#3d2466"}`,
...
function PizzaTracker({ reviewCount, onAddReview }) {
  ...
  return (
    <div style={{
      background: "#1a0b2e", border: "1px solid #4a2f7a", borderRadius: 4, padding: 20, marginBottom: 20,
      display: "flex", alignItems: "center", gap: 20, flexWrap: "wrap"
    }}>
      <svg width="110" height="110" viewBox="0 0 120 120" style={{ flexShrink: 0 }}>
        <circle cx={cx} cy={cy} r={r} fill="#0d0221" stroke="#4a2f7a" strokeWidth="2" />
        {Array.from({ length: SLICES_PER_PIZZA }).map((_, i) => (
          <path key={i} d={wedgePath(i, SLICES_PER_PIZZA, cx, cy, r - 2)} fill={i < currentSlices ? "#ffd23f" : "transparent"} stroke="#4a2f7a" strokeWidth="1.5" />
        ))}
      </svg>
      <div style={{ flex: 1, minWidth: 180 }}>
        <div style={{ fontFamily: "'Press Start 2P', cursive", fontSize: 16, color: "#fffbe8", marginBottom: 4 }}>Pizza progress</div>
...
function EditReviewForm({ review, restaurant, onSave, onCancel }) {
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
        <button onClick={onCancel} style={{ background: "none", border: "none", color: "#8b8bc4", cursor: "pointer" }}><ArrowLeft size={18} /></button>
        <div>
          <div style={{ fontFamily: "'Press Start 2P', cursive", fontSize: 19, color: "#fffbe8" }}>Edit review</div>
          <div style={{ fontSize: 12, color: "#8b8bc4" }}>{restaurant ? restaurant.name : "Unknown restaurant"}</div>
        </div>
      </div>
...
function ManageReviews({ restaurants, reviews, onUpdateReview, onDeleteReview, onClose }) {
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
        <div style={{ fontFamily: "'Press Start 2P', cursive", fontSize: 20, color: "#fffbe8" }}>Manage reviews</div>
        <button onClick={onClose} style={{ background: "none", border: "none", color: "#8b8bc4", cursor: "pointer" }}><X size={20} /></button>
      </div>

      {sorted.length === 0 ? (
        <div style={{ color: "#8b8bc4", fontSize: 13, textAlign: "center", padding: "30px 0" }}>No reviews yet.</div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: 10, maxHeight: 480, overflowY: "auto" }}>
          {sorted.map(rv => {
            const restaurant = restaurants.find(r => r.id === rv.restaurantId);
            return (
              <div key={rv.id} style={{ background: "#0d0221", border: "1px solid #3d2466", borderRadius: 3, padding: 14 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10 }}>
                  <div style={{ minWidth: 0 }}>
                    <div style={{ color: "#fffbe8", fontSize: 14, fontWeight: 600 }}>{restaurant ? restaurant.name : "Unknown restaurant"}</div>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 3 }}>
                      <StarRating value={rv.starRating} readOnly size={13} />
                      <span style={{ fontSize: 11, color: "#8b8bc4" }}>{new Date(rv.timestamp).toLocaleDateString()}</span>
                    </div>
                    <div style={{ fontSize: 12, color: "#8b8bc4", marginTop: 6 }}>
                      {rv.reviewText.length > 130 ? rv.reviewText.slice(0, 130) + "…" : rv.reviewText}
                    </div>
                  </div>
                  <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
                    <button onClick={() => setEditingId(rv.id)} style={{
                      background: "none", border: "1px solid #4a2f7a", borderRadius: 2, padding: 7, cursor: "pointer", color: "#8b8bc4"
                    }}><Edit2 size={14} /></button>
                    <button onClick={() => setConfirmDeleteId(rv.id)} style={{
                      background: "none", border: "1px solid #ff2e63", borderRadius: 2, padding: 7, cursor: "pointer", color: "#ff2e63"
                    }}><Trash2 size={14} /></button>
                  </div>
                </div>
                {confirmDeleteId === rv.id && (
                  <div style={{
                    marginTop: 10, background: "#2a0d10", border: "1px solid #ff2e63", borderRadius: 2, padding: 10,
                    display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, flexWrap: "wrap"
                  }}>
                    <span style={{ fontSize: 12, color: "#ff85a8" }}>Delete this review permanently?</span>
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14 }}>
        ...
        <div style={{ fontFamily: "'Press Start 2P', cursive", fontSize: 18, color: "#fffbe8" }}>Top 5 recommended</div>
      </div>
      {top5.length === 0 ? (
        <EmptyState />
      ) : (
      ...
      {cuisinesWithData.length > 0 && (
        <>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 14 }}>
            ...
            <div style={{ fontFamily: "'Press Start 2P', cursive", fontSize: 18, color: "#fffbe8" }}>Top picks by cuisine</div>
          </div>
          {cuisinesWithData.map(c => (
            <div key={c} style={{ marginBottom: 22 }}>
              <div style={{ fontSize: 13, color: "#8b8bc4", marginBottom: 8, fontWeight: 500 }}>{c}</div>
...
function EmptyState() {
  return (
    <div style={{
      border: "1px dashed #4a2f7a", borderRadius: 4, padding: 30, textAlign: "center", marginBottom: 32
    }}>
      <Heart size={26} color="#5c5470" style={{ marginBottom: 8 }} />
      <div style={{ color: "#8b8bc4", fontSize: 14 }}>No reviews yet. Once the first one lands, the leaderboard fills in here.</div>
    </div>
  );
}
...
export default function NushNom() {
  ...
  if (!data) {
    return (
      <div style={{ minHeight: 300, display: "flex", alignItems: "center", justifyContent: "center", background: "#0d0221" }}>
  ...
  return (
    <div style={{
      background: "#0d0221", minHeight: "100vh", padding: "28px 20px",
      ...
      <div style={{ maxWidth: 760, margin: "0 auto" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, marginBottom: 24, flexWrap: "wrap" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{
              width: 40, height: 40, borderRadius: 3, background: "#ff2e63",
              display: "flex", alignItems: "center", justifyContent: "center"
            }}>
              <ChefHat size={22} color="#3d0d1f" />
            </div>
            <div>
              <div style={{ fontFamily: "'Press Start 2P', cursive", fontSize: 24, color: "#fffbe8", fontWeight: 800, lineHeight: 1 }}>NushNom</div>
              <div style={{ fontSize: 11, color: "#8b8bc4" }}>Anushka's Mumbai food quest</div>
            </div>
          </div>
          {view === "dashboard" && (
            <button onClick={() => requestAccess("manage")} style={{
              background: "none", border: "1px solid #4a2f7a", borderRadius: 2, padding: "8px 12px",
              color: "#8b8bc4", fontSize: 12, cursor: "pointer", display: "flex", alignItems: "center", gap: 6
            }}>
        ...
        {view === "add" && (
          <div style={{ background: "#1a0b2e", border: "1px solid #4a2f7a", borderRadius: 4, padding: 22 }}>
            <AddReviewFlow
