...
function PasscodeGate({ onUnlock, onCancel }) {
  ...
  return (
    <div style={{
      background: "#241640", border: "1px solid #4E3480", borderRadius: 16,
      padding: 28, maxWidth: 340, margin: "40px auto", textAlign: "center"
    }}>
      <Lock size={28} color="#FFB627" style={{ marginBottom: 10 }} />
      <div style={{ fontFamily: "'Baloo 2', cursive", fontSize: 18, color: "#F5EFE6", marginBottom: 6 }}>Curator access only</div>
      <div style={{ fontSize: 13, color: "#B8A9D9", marginBottom: 16 }}>Enter the passcode to add a review.</div>
      <input
        type="password"
        value={val}
        ...
        style={{
          width: "100%", padding: "10px 12px", borderRadius: 12, border: `1px solid ${err ? "#FF4D8D" : "#4E3480"}`,
          background: "#1B1030", color: "#F5EFE6", fontSize: 14, marginBottom: 14, boxSizing: "border-box"
        }}
        onKeyDown={e => { if (e.key === "Enter") { val === PASSCODE ? onUnlock() : setErr(true); } }}
      />
      {err && <div style={{ color: "#FF4D8D", fontSize: 12, marginBottom: 10 }}>Wrong passcode, try again.</div>}
        ...
        <>
          ...
          {cuisinesWithData.map(c => (
            <div key={c} style={{ marginBottom: 22 }}>
              <div style={{ fontSize: 13, color: "#B8A9D9", marginBottom: 8, fontWeight: 500 }}>{c}</div>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, marginBottom: 24, flexWrap: "wrap" }}>
          ...
          {view === "dashboard" && (
            <button onClick={() => requestAccess("manage")} style={{
              background: "none", border: "1px solid #4E3480", borderRadius: 10, padding: "8px 12px",
              color: "#B8A9D9", fontSize: 12, cursor: "pointer", display: "flex", alignItems: "center", gap: 6
            }}>
